# = Para N ÍMPAR - Lógica do Quadrado Mágico = #
def caso1(n: int) -> list:
    m = [[0] * n for i in range(n)]
    i = 0
    j = len(m) // 2
    for k in range(1, (len(m) ** 2) + 1):        
        m[i][j] = k
        i -= 1
        j += 1
        if k % len(m) == 0:
...
def caso2(n: int) -> list:
    ...
    for i in range(0, len(m), 4):
        for j in range(0, len(m[0]), 4):
            sub_m = matriz4x4(m, n, i, j)
            for k in range(len(sub_m)):
                for l in range(len(sub_m[0])):
                    if m[k+i][l+j] != sub_m[k][l]:
                        m[k+i][l+j] = sub_m[k][l]
    return m
